// This file was generated by prisma-field-encryption.

import type { PrismaClient, StatementLine } from '@prisma/client'
import {
  ProgressReportCallback,
  defaultProgressReport,
  visitRecords
} from 'prisma-field-encryption/dist/generator/runtime'

type Cursor = StatementLine['id']

export async function migrate(
  client: PrismaClient,
  reportProgress: ProgressReportCallback = defaultProgressReport
): Promise<number> {
  return visitRecords<PrismaClient, Cursor>({
    modelName: 'StatementLine',
    client,
    getTotalCount: client.statementLine.count,
    migrateRecord,
    reportProgress,
  })
}

async function migrateRecord(client: PrismaClient, cursor: Cursor | undefined) {
  return await client.$transaction(async tx => {
    const record = await tx.statementLine.findFirst({
      take: 1,
      skip: cursor === undefined ? undefined : 1,
      ...(cursor === undefined
        ? {}
        : {
            cursor: {
              id: cursor
            }
          }),
      orderBy: {
        id: 'asc'
      },
      select: {
        id: true,
        description: true
      }
    })
    if (!record) {
      return cursor
    }
    await tx.statementLine.update({
      where: {
        id: record.id
      },
      data: {
        description: record.description
      }
    })
    return record.id
  })
}

/**
 * Internal model:
 * {
 *   "cursor": "id",
 *   "fields": {
 *     "description": {
 *       "encrypt": true,
 *       "strictDecryption": false
 *     }
 *   },
 *   "connections": {
 *     "period": {
 *       "modelName": "ReconciliationPeriod",
 *       "isList": false
 *     },
 *     "registerEntry": {
 *       "modelName": "RegisterEntry",
 *       "isList": false
 *     }
 *   }
 * }
 */
