...
const FIXTURE_WEAK_PASSWORD = "123"; // NOSONAR S2068 — fixture
...
describe("Additional API Endpoints Coverage", () => {
  ...
  describe("GET /api/validate-token", () => {
    ...
    it("should handle database errors", async () => {
      ...
      await expect(validateTokenHandler(mockEvent)).rejects.toThrow(
        "Database error",
      );
  ...
  describe("POST /api/change-password", () => {
    ...
    it("should successfully change password and send notification", async () => {
      ...
      expect(prisma.user.update).toHaveBeenCalledWith({
        where: { id: 123 },
        data: { password: FIXTURE_HASHED_NEW_PASSWORD },
      });
  ...
  describe("POST /api/user", () => {
    ...
    it("should handle user not found error", async () => {
      ...
      await expect(userPostHandler(mockEvent)).rejects.toThrow(
        "User not found",
      );
