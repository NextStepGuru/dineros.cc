...
describe("Server Library Functions", () => {
  ...
  describe("handleApiError", () => {
    ...
    it("should handle Zod validation errors", () => {
      ...
      expect(() => handleApiError(zodError)).toThrow(
        "HTTP 400: name: Expected string, received number, age: Expected number, received string",
      );

      expect(mockCreateError).toHaveBeenCalledWith({
        statusCode: 400,
        statusMessage:
          "name: Expected string, received number, age: Expected number, received string",
      });
    });
    ...
    it("should handle Prisma database errors", () => {
      ...
      expect(() => handleApiError(prismaError)).toThrow(
    ...
    it("should handle PrismaClientInitializationError with 503", () => {
      ...
      expect(() => handleApiError(initError)).toThrow(
  ...
  describe("getUser", () => {
    ...
    it("should throw error when user is not in context", () => {
      ...
      expect(() => getUser(mockEvent as any)).toThrow(
        "HTTP 401: User not found in context",
      );
  ...
  describe("updateMulti", () => {
    ...
    it("should properly escape field names in SQL", async () => {
      ...
      const expectedSQL =
        'UPDATE "weird_table" SET "field-with-dashes" = "t"."field-with-dashes", "field.with.dots" = "t"."field.with.dots" FROM (VALUES (\'$1\',\'$2\',\'$3\')) AS t("id", "field-with-dashes", "field.with.dots") WHERE "weird_table"."id" = "t"."id"';
    ...
    it("should handle database errors gracefully", async () => {
      ...
      await expect(
        updateMulti(mockPrisma, tableName, fields, values),
  ...
  describe("Integration Tests", () => {
    it("should work together in typical API flow", async () => {
      ...
      expect(() => handleApiError(validationError)).toThrow(
        "HTTP 400: accountId: Expected string, received number",
      );
    });
  });
});
