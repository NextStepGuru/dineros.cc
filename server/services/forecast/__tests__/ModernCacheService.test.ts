...
import type { CacheAccountRegister, CacheRegisterEntry, CacheReoccurrence, CacheReoccurrenceSkip } from '../ModernCacheService';
...
describe('ModernCacheService', () => {
  ...
  function createMockAccountRegister(overrides: Partial<CacheAccountRegister> = {}): CacheAccountRegister {
    return {
      id: 1,
      typeId: 1,
      budgetId: 1,
  ...
  function createMockRegisterEntry(overrides: Partial<CacheRegisterEntry> = {}): CacheRegisterEntry {
    return {
      id: 'entry-1',
      accountRegisterId: 1,
  ...
  function createMockReoccurrence(overrides: Partial<CacheReoccurrence> = {}): CacheReoccurrence {
    return {
      id: 1,
      accountId: 'test-account',
      accountRegisterId: 1,
  ...
  function createMockReoccurrenceSkip(overrides: Partial<CacheReoccurrenceSkip> = {}): CacheReoccurrenceSkip {
    return {
      id: 1,
      reoccurrenceId: 1,
      accountId: 'test-account',
  ...
  describe('ModernCollection operations', () => {
    describe('insert and insertMany', () => {
      it('should insert single item', () => {
        const account = createMockAccountRegister({ id: 1, name: 'Test Account' });
        cache.accountRegister.insert(account);
    ...
    describe('find operations', () => {
      beforeEach(() => {
        cache.accountRegister.insertMany([
          createMockAccountRegister({ id: 1, name: 'Checking', typeId: 1, balance: 1000 }),
          createMockAccountRegister({ id: 2, name: 'Savings', typeId: 2, balance: 5000 }),
          createMockAccountRegister({ id: 3, name: 'Credit Card', typeId: 3, balance: -500 }),
        ]);
      });
      ...
      it('should find items by function predicate', () => {
        const positiveBalance = cache.accountRegister.find(acc => acc.balance > 0);
    ...
    describe('update operations', () => {
      it('should update single item', () => {
        const account = createMockAccountRegister({ id: 1, name: 'Original', balance: 1000 });
        cache.accountRegister.insert(account);
    ...
    describe('remove operations', () => {
      ...
      it('should remove items by function predicate', () => {
        ...
        const removedCount = cache.accountRegister.remove(acc => acc.balance < 0);
  ...
  describe('ChainableCollection operations', () => {
    beforeEach(() => {
      cache.registerEntry.insertMany([
        createMockRegisterEntry({ id: 'entry-1', accountRegisterId: 1, amount: 100, createdAt: moment('2024-01-01') }),
        createMockRegisterEntry({ id: 'entry-2', accountRegisterId: 1, amount: 200, createdAt: moment('2024-01-02') }),
        createMockRegisterEntry({ id: 'entry-3', accountRegisterId: 2, amount: 50, createdAt: moment('2024-01-03') }),
        createMockRegisterEntry({ id: 'entry-4', accountRegisterId: 1, amount: 300, createdAt: moment('2024-01-04') }),
      ]);
    });
    ...
    it('should chain find operations', () => {
      ...
      expect(result.every(entry => entry.accountRegisterId === 1 && entry.amount > 150)).toBe(true);
    });
    ...
    it('should sort strings correctly', () => {
      ...
      const result = cache.accountRegister
        .chain()
        .simplesort('name')
        .data();
  ...
  describe('Complex query operators', () => {
    beforeEach(() => {
      cache.registerEntry.insertMany([
        createMockRegisterEntry({ id: 'entry-1', amount: 100, isPending: true }),
        createMockRegisterEntry({ id: 'entry-2', amount: 200, isPending: false }),
        createMockRegisterEntry({ id: 'entry-3', amount: 300, isPending: true }),
        createMockRegisterEntry({ id: 'entry-4', amount: 150, isPending: false }),
      ]);
    });
    ...
    it('should handle $in operator', () => {
      const result = cache.registerEntry.find({ amount: { $in: [100, 300] } } as any);
      expect(result).toHaveLength(2);
      expect(result.every(entry => [100, 300].includes(entry.amount))).toBe(true);
    });

    it('should handle $nin operator', () => {
      const result = cache.registerEntry.find({ amount: { $nin: [100, 300] } } as any);
      expect(result).toHaveLength(2);
      expect(result.every(entry => ![100, 300].includes(entry.amount))).toBe(true);
    });

    it('should handle $and operator with function predicate', () => {
      const result = cache.registerEntry.find(entry =>
        entry.amount > 100 && entry.isPending === true
      );
      expect(result).toHaveLength(1);
      expect(result[0].amount).toBe(300);
    ...
    it('should handle $or operator with function predicate', () => {
      const result = cache.registerEntry.find(entry =>
        entry.amount === 100 || entry.amount === 300
      );
      expect(result).toHaveLength(2);
      expect(result.every(entry => [100, 300].includes(entry.amount))).toBe(true);
    });
    ...
    it('should handle complex nested logic with function predicate', () => {
      const result = cache.registerEntry.find(entry =>
        (entry.amount < 150 || entry.amount > 250) && entry.isPending === true
      );
      expect(result).toHaveLength(2);
      expect(result.every(entry => entry.isPending === true)).toBe(true);
      expect(result.every(entry => entry.amount < 150 || entry.amount > 250)).toBe(true);
    });
  });
  ...
  describe('Index management', () => {
    ...
    it('should update items but may not clean up old index entries', () => {
      const account = createMockAccountRegister({ id: 1, accountId: 'original' });
      cache.accountRegister.insert(account);

      expect(cache.accountRegister.find({ accountId: 'original' })).toHaveLength(1);

      const updated = { ...account, accountId: 'updated' };
      cache.accountRegister.update(updated);

      // The updated item should be findable by new accountId
      expect(cache.accountRegister.find({ accountId: 'updated' })).toHaveLength(1);
    ...
    it('should remove from indexes when items are removed', () => {
      ...
      expect(cache.accountRegister.find({ accountId: 'test-1' })).toHaveLength(1);

      cache.accountRegister.remove({ id: 1 });

      expect(cache.accountRegister.find({ accountId: 'test-1' })).toHaveLength(0);
      expect(cache.accountRegister.find({ accountId: 'test-2' })).toHaveLength(1);
    });
  });
  ...
  describe('Edge cases', () => {
    it('should handle empty collections', () => {
      ...
      expect(cache.accountRegister.updateMany({ id: 1 }, { name: 'test' })).toBe(0);
      expect(cache.accountRegister.remove({ id: 1 })).toBe(0);
    });

    it('should handle queries with no matches', () => {
      cache.accountRegister.insert(createMockAccountRegister({ id: 1, name: 'Test' }));
    ...
    it('should handle updates on non-existent items', () => {
      const nonExistent = createMockAccountRegister({ id: 999, name: 'Non-existent' });
    ...
    it('should handle complex data types in sort', () => {
      cache.registerEntry.insertMany([
        createMockRegisterEntry({ id: 'entry-1', createdAt: moment('2024-01-03') }),
        createMockRegisterEntry({ id: 'entry-2', createdAt: moment('2024-01-01') }),
        createMockRegisterEntry({ id: 'entry-3', createdAt: moment('2024-01-02') }),
      ]);
