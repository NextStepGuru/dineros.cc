...
describe("Double Interest Calculation Regression Tests", () => {
  ...
  describe("LoanCalculatorService.shouldProcessInterest()", () => {
    it("should return true only on exact statement date", () => {
      ...
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-18")),
      ).toBe(false);
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-20")),
      ).toBe(false);
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-22")),
    ...
    it("should not process interest if no APR", () => {
      ...
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-15")),
      ).toBe(false);
    });
    ...
    it("should calculate next statement date correctly for different intervals", async () => {
      ...
      for (const testCase of testCases) {
        ...
        const account = {
          ...
          balance: -1000,
          latestBalance: -1000,
          statementAt: dt(testCase.current).toDate(),
  ...
  describe("Interest processing timeline simulation", () => {
    it("should process interest exactly once per month over multiple months", () => {
      ...
      let currentDate = dt("2025-01-01");
      const endDate = dt("2025-04-01");
      ...
      for (let i = 1; i < interestDates.length; i++) {
        const current = dt(interestDates[i]);
        const previous = dt(interestDates[i - 1]);
  ...
  describe("Integration test: Full interest processing cycle", () => {
    it("should process interest once per statement period in a full forecast", async () => {
      ...
      for (let month = 0; month < 4; month++) {
        const statementDate = dt("2025-02-01").add(month, "months");
  ...
  describe("Edge cases that could trigger double processing", () => {
    ...
    it("should handle month-end/month-start boundaries correctly", () => {
      ...
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-31")),
      ).toBe(true);
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-01-30")),
      ).toBe(false);
      expect(
        loanCalculator.shouldProcessInterest(account, dt("2025-02-01")),
      ).toBe(false);
    });
  });
});
