...
describe("Budget API Endpoints", () => {
  ...
  describe("POST /api/budget", () => {
    ...
    it("returns 201 and budget when default exists and under limit", async () => {
      ...
      const { prisma } = await import("~/server/clients/prismaClient");
      const { cloneBudget } = await import("~/server/services/budgetCloneService");
  ...
  describe("DELETE /api/budget/:id", () => {
    ...
    beforeEach(async () => {
      ...
      prisma.$transaction.mockImplementation(async (cb: (tx: any) => Promise<any>) => {
        const tx = {
          accountRegister: { updateMany: vi.fn().mockResolvedValue({ count: 0 }) },
  ...
  describe("POST /api/budget/:id/reset", () => {
    ...
    beforeEach(async () => {
      ...
      const mockTx = {
        ...
        reoccurrencePlaidNameAlias: { deleteMany: vi.fn().mockResolvedValue({ count: 0 }) },
        reoccurrenceSplit: { deleteMany: vi.fn().mockResolvedValue({ count: 0 }) },
        reoccurrenceSkip: { deleteMany: vi.fn().mockResolvedValue({ count: 0 }) },
      };
      prisma.$transaction.mockImplementation(async (cb: (tx: any) => Promise<any>) =>
        cb(mockTx)
      );
